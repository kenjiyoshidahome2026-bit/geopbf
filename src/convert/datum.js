...
export async function resolveDatum(opts = {}) {
	const one = async (src, want) => {
		...
		else { try { g = await loadMeshGrid(src); } catch (e) { console.warn(`[datum] ${want} grid could not be loaded; continuing with the approximation: ${e.message}`); return null; } }
		if (g.transform && g.transform !== want) { console.warn(`[datum] ${want} was given a ${g.transform} grid; ignored`); return null; }
		return g;
	};
