...
// 連続 id の同一内容は runLength に畳む（海の空タイル等）。内部圧縮・タイル圧縮は gzip（pako＝既存依存）。
...
const decompress = (u8, code) => code === 2 ? pako.ungzip(u8) : u8;
const hash = (u8) => {   // FNV-1a 32 ×2（異なる種）＋長さ＝重複タイルの候補キー。衝突は bytes 比較で確定
...
//         compress: (Uint8Array)=>Uint8Array（gzip の差し替え・既定 pako） }
export function writePMTiles(tiles, metadata = {}, opts = {}) {
	const tc = opts.tileCompression ?? "gzip", ic = opts.internalCompression ?? "gzip";
	const tileType = opts.tileType ?? 1;
	// 1) id 付与・整列・重複畳み込み。ハッシュは「圧縮前」のバイト列で取る＝同じ内容（内陸の全面塗りタイル・海の
	//    空タイル等）は 1 回しか圧縮しない。圧縮は差し替え可（Node の zlib は pako の数倍速い＝tiler が注入する）。
	const gz = opts.compress ?? ((u8) => pako.gzip(u8));
	const compress = (u8, kind) => kind === "gzip" ? gz(u8) : u8;
	const items = tiles.map(t => ({ id: zxyToTileId(t.z, t.x, t.y), data: t.data }));
	items.sort((a, b) => a.id - b.id);
	const chunks = [], byHash = new Map();
	let dataLen = 0;
	const entries = [];
	for (const it of items) {
		const h = hash(it.data);
		let rec = null;
		const cands = byHash.get(h);
		if (cands) for (const c of cands) if (same(c.raw, it.data)) { rec = c; break; }
		if (!rec) {
			const comp = compress(it.data, tc);
			rec = { raw: it.data, offset: dataLen, length: comp.length }; dataLen += comp.length; chunks.push(comp);
			if (cands) cands.push(rec); else byHash.set(h, [rec]);
		}
	...
	const rootOnly = compress(serializeDirectory(entries), ic);
	if (rootOnly.length <= ROOT_MAX) { root = entries; rootBytes = rootOnly; }
		...
		for (let leafSize = 4096; ; leafSize *= 2) {
			...
			for (let i = 0; i < entries.length; i += leafSize) {
				const leaf = compress(serializeDirectory(entries.slice(i, i + leafSize)), ic);
				rootEntries.push({ tileId: entries[i].tileId, offset: off, length: leaf.length, runLength: 0 });
				parts.push(leaf); off += leaf.length;
			}
			const rb = compress(serializeDirectory(rootEntries), ic);
	...
	// 3) メタ・ヘッダ
	const metaBytes = compress(new TextEncoder().encode(JSON.stringify(metadata)), ic);
	...
	header[100] = opts.minZoom ?? (zs.length ? Math.min(...zs) : 0);
	header[101] = opts.maxZoom ?? (zs.length ? Math.max(...zs) : 0);
...
// ── 検定用リーダ（同期・全量メモリ） ──
export function readPMTiles(u8) {
	...
	const root = deserializeDirectory(decompress(u8.subarray(h.rootOff, h.rootOff + h.rootLen), h.ic));
	const metadata = JSON.parse(new TextDecoder().decode(decompress(u8.subarray(h.metaOff, h.metaOff + h.metaLen), h.ic)));
	...
	const getTile = (z, x, y) => {
		...
		let e = find(root, id);
		if (e && e.runLength === 0) { const leaf = deserializeDirectory(decompress(u8.subarray(h.leafOff + e.offset, h.leafOff + e.offset + e.length), h.ic)); e = find(leaf, id); if (e && e.runLength === 0) return null; }
		if (!e) return null;
