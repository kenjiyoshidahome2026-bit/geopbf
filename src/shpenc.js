async function shpenc(arraybuffer, name, encoding, level = 3) { encoding = (encoding||"utf8").toLowerCase().replace(/[\-\_]/g,"").replace(/shiftjis/,"sjis");
//	#inline("4MBhnpea");// iconv-lite
	importScripts(`https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js`);
	importScripts(`https://cdnjs.cloudflare.com/ajax/libs/encoding-japanese/2.0.0/encoding.min.js`);
	importScripts(`https://cdn.jsdelivr.net/npm/pbf@4.0.1/dist/pbf.min.js`);
	// #inline("DXUxnJT7");// Logger
	// #inline("s42GcdLp");//PBF
...
////=======================================================================================================================
	...
	shpTypes.forEach(([shpType, shpCode], i)=>{ if (!types[i].length) return;
		...
		writeShp(fname, types[i], shpCode);
		writeDbf(fname, types[i]);
		zip.file(fname + '.prj', prj);
		zip.file(fname + '.cpg', encoding);
	});
	...
	const blob = await zip.generateAsync({type:"blob", compression:"DEFLATE", compressionOptions:{level}}, update);
//	console.log(blob)
...
////=======================================================================================================================
	function writeShp(name, farray, type) {
		...
		zip.file(name + '.shp', SHP.buffer(), {binary:true});
		zip.file(name + '.shx', SHX.buffer(), {binary:true});
...
////--------------------------------------------------------------------------------------------------------------------------	
	function writeDbf(name, farray) {
		...
		DBF.writeUint8(0x1a);
		zip.file(name + '.dbf', DBF.buffer(), {binary:true});
	}
////--------------------------------------------------------------------------------------------------------------------------	
}