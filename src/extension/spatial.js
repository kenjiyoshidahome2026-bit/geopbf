...
export const area = (self, i) => {
    ...
    const ringArea = coords => {
        ...
        if (n > 2) {
            for (let j = 0; j < n; j++) {
                let p1 = coords[j === 0 ? n - 1 : j - 1], p2 = coords[j], p3 = coords[j === n - 1 ? 0 : j + 1];
    ...
    const calc = (g) => {
        if (g.type === "Polygon") {
            total += ringArea(g.coordinates[0]);
            for (let j = 1; j < g.coordinates.length; j++) total -= ringArea(g.coordinates[j]);
...
export const contain = (self, [px, py], getOneFlag) => {
    ...
    const isContain = n => {
        ...
        if (type < 4 || out(self.getBbox(n))) return false;
    ...
    const a = [];
    for (let i = 0; i < self.length; i++) if (isContain(i)) { if (getOneFlag) return i; a.push(i); }
