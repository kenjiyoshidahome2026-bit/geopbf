...
export async function encodeXLSX(src, name = null, { sheetName = 'Sheet1' } = {}) {
	...
	if (rows.length > MAX_ROWS) { console.warn(`[encodeXLSX] ${rows.length} rows truncated to the Excel limit of ${MAX_ROWS}`); rows = rows.slice(0, MAX_ROWS); }
	let nCols = rows.reduce((m, r) => Math.max(m, r.length), 0);
	if (nCols > MAX_COLS) { console.warn(`[encodeXLSX] ${nCols} columns truncated to the Excel limit of ${MAX_COLS}`); rows = rows.map(r => r.slice(0, MAX_COLS)); nCols = MAX_COLS; }
	// シート名の禁則: : \ / ? * [ ] と 31 文字上限
